function buildLocalAdvisory(message, location, weather, language = 'en') {
  ...
  if (language === 'hi') {
    if (['rain', 'umbrella', 'wet', 'drizzle', 'shower', 'बारिश', 'छाता', 'बरसात'].some((k) => q.includes(k))) {
      const raining = rainMm > 0 || rainProb > 50 || /rain|drizzle|shower/i.test(desc);
      return (
        `🌧️ **${cityName} के लिए बारिश का पूर्वानुमान:**\n\n` +
        `वर्तमान स्थिति: **${desc}** · वर्षा: **${rainMm} mm** · संभावना: **${rainProb}%**\n\n` +
        `${raining ? '⚠️ बारिश की पूरी संभावना है — बाहर निकलते समय छाता साथ रखें।' : '✅ अभी तेज बारिश की संभावना कम है, पर 24 घंटे का पूर्वानुमान देखते रहें।'}\n\n` +
        `हवा की गति **${wind} km/h** है — तेज हवा में छोटा छाता उपयोगी रहेगा।`
      );
    }
    return (
      ...
      'आप मुझसे बारिश, कपड़ों के चयन, खेती, यूवी इंडेक्स या हवा के बारे में पूछ सकते हैं।'
    );
  }

  if (language === 'bn') {
    if (['rain', 'umbrella', 'wet', 'drizzle', 'shower', 'বৃষ্টি', 'ছাতা'].some((k) => q.includes(k))) {
      const raining = rainMm > 0 || rainProb > 50 || /rain|drizzle|shower/i.test(desc);
      return (
        `🌧️ **${cityName}-এর জন্য বৃষ্টির পূর্বাভাস:**\n\n` +
        `বর্তমান অবস্থা: **${desc}** · বৃষ্টিপাত: **${rainMm} mm** · সম্ভাবনা: **${rainProb}%**\n\n` +
        `${raining ? '⚠️ বৃষ্টির প্রবল সম্ভাবনা রয়েছে — সাথে ছাতা রাখুন।' : '✅ আপাতত ভারী বৃষ্টির সম্ভাবনা কম।'}\n\n` +
    ...
    return (
      ...
      'আপনি আমাকে বৃষ্টি, জামাকাপড় বা কৃষিকাজ সম্পর্কে প্রশ্ন করতে পারেন।'
    );
  }
